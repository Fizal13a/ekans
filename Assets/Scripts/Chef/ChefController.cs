...
public class ChefController : MonoBehaviour
{
    ...
    private float minThrowTime = 3f;

    [SerializeField] private float maxThrowTime = 6f;
    [SerializeField] private int amountToThrow = 3;
    ...
    {
        GameManager.events.AddEvent(GameEvents.EventType.OnGameOver, StopThrowing);
    }
    ...
    private void Start()
    {
        ...
        throwRoutine = StartCoroutine(ThrowFoodRoutine());
    }
        ...
        while (true)
        {
            ...
            for (int i = 0; i < amountToThrow; i++)
            {
                ...
                ParticleSystem particle =
                    GetRandomParticle(particlesToUse);
    ...
    private void ThrowObject(Transform food, Vector3 targetPosition)
    {
        ...
        Vector3 baseScale = foodOriginalScales.TryGetValue(food.gameObject, out Vector3 s)
            ? s
            : food.localScale;
        ...
        landingSequence.Append(
            food.DOScale(
                new Vector3(baseScale.x * landingSquashWiden, baseScale.y * landingSquashFlatten,
                    baseScale.z * landingSquashWiden),
                landingSquashDuration).SetEase(Ease.OutQuad));

        landingSequence.Append(
            food.DOScale(baseScale, landingRecoverDuration).SetEase(Ease.OutElastic));
    }
    ...
    private void DisableFoodWithJuice(GameObject food)
    {
        ...
        t.DOKill();
        t.DOScale(Vector3.zero, disableScaleDuration).SetEase(Ease.InBack).OnComplete(() =>
        {
    ...
    private void DisableParticleWithJuice(ParticleSystem particle)
    {
        ...
        t.DOKill();
        t.DOScale(Vector3.zero, particlePopOutDuration).SetEase(Ease.InBack).OnComplete(() =>
        {
            particle.Stop();
    ...
    private GameObject GetRandomInactiveFood(
        List<GameObject> alreadySelected)
    {
        List<GameObject> availableFoods =
            new List<GameObject>();

        foreach (GameObject food in foodObjects)
        {
            if (!food.activeSelf &&
                !alreadySelected.Contains(food))
            {
        ...
        return availableFoods[
            Random.Range(0, availableFoods.Count)
        ];
    }

    private ParticleSystem GetRandomParticle(
        List<ParticleSystem> alreadySelected)
    {
        List<ParticleSystem> availableParticles =
            new List<ParticleSystem>();

        foreach (ParticleSystem particle in spawnParticles)
        {
            if (!particle.gameObject.activeSelf &&
                !alreadySelected.Contains(particle))
            {
        ...
        return availableParticles[
            Random.Range(0, availableParticles.Count)
        ];
    }
    ...
    {
        GameManager.events.RemoveEvent(GameEvents.EventType.OnGameOver, StopThrowing);
    }
